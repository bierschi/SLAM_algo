...
#include "urg_connection.h"
    ...
    */
    ...
    /*!
     * @brief Communication with the lidar and say it to start with specified action (with param).
     * @param[in] urg the urg_t sensor struct which is a deputy for the sensor
     * @param[in] type one of the @see urg_measurement_type
     * @param[in] scan_times specifies how often should the sensor send lidar measurements back. 0 = URG_SCAN_INFINITY means to send forever measurements
     * @param[in] skip_scan means that every skip_scan'th measurement is not been sending to the host
