...
#include "urg_sensor.h"
    ...
    */
    ...
    /*!
     * @brief calculates the degree value for a given step
     * @param [in] urg the urg sensor for the calculation
     * @param [in] step the step for the degree which should be calculated e.g. 25
     * @param [out] degree the degree value or
     */
    extern double urg_step2deg(const urg_t *urg, int step);

    /*!
     * @brief calculates for a specific step the index, where the measurement of this step will being in the data array
     * @param [in] urg the urg sensor for which the step should be calculated
     * @param [in] step the step of which the index should be calculated
     * @param [out] the index in the data array or the error code URG_NOT_CONNECTED if the sensor is not connected at the moment
     */
